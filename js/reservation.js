...
function proveriVreme() {
	...
	if (now.getTime() >= date.getTime()) {
		...
		"<strong>Не можете резервисати за прошлост!</strong></div>";
		ok = false;
	}
	else if ((date.getTime() - now.getTime()) > (1000 * 3600 * 24 * 30)) {
		document.getElementById("timeWarning").innerHTML = "<div class='alert alert-danger bo-rad-10'>" + 
		"<strong>Не можете резервисати више од месец дана унапред!</strong></div>";
		ok = false;
	}
	else {
...
function proveriFilm() {
	...
	if (document.getElementById("movie").selectedIndex == 0) {
		...
		"<strong>Нисте изабрали филм!</strong></div>";
		ok = false;
	}
	else
...
function proveriImejl() {
	...
	if (email.length == 0) {
		...
		"<strong>Нисте унели мејл!</strong></div>";
		ok = false;
	}
	else if (!emailCheck.test(email)) {
		document.getElementById("emailWarning").innerHTML = "<div class='size12 bo-rad-10 alert alert-danger'>" + 
		"<strong>Лош формат!</strong></div>";
		ok = false;
	}
	else
...
function proveriTelefon() {
	...
	if (phone.length == 0) {
		...
		"<strong>Нисте унели телефон!</strong></div>";
		ok = false;
	}
...
function izlistajRezervacije() {
	//inicijalizuj();
	if (!proveriBrowser()) {
		document.getElementById("lista").innerHTML = "<div class='container section-mainmenu p-t-110 p-b-70 bg1-pattern'>Ваш претраживач не подржава ову функционалност!</div>";
		return;
	}
	var ukupno = localStorage.getItem("rezervacije");
	if ((ukupno == null) || (ukupno == 0)) {
		document.getElementById("lista").innerHTML = "<div class='container section-mainmenu p-t-110 p-b-70 bg1-pattern'>Још нисте направили ни једну резервацију!</div>";
		return;
	}
	var niz = new Array();
				...
				"<div class='col-md-8 col-lg-6 m-l-r-auto'>" +
					"<div class='blo3 flex-w flex-col-l-sm m-b-30'>" +
						"<div class='pic-blo3 size20 bo-rad-10 hov-img-zoom m-r-28'>" +
							"<a href='" + podaci[0] + ".html" + "'><img src='" + podaci[6] + "' alt='IMG-MENU' height='80%'></a>" +
						"</div>" +
						"<div class='text-blo3 size21 flex-col-l-m'>" +
							"<a href='" + podaci[0] + ".html" + "' class='txt22 m-b-3'>" +
								podaci[1] +
							"</a>" + 
							"<div><b>Трајање: </b>" + podaci[7] + "</div>" +
							"<div><b>Режисер: </b>" + podaci[8] + "</div>" +
							"<div><b>Улоге: </b>" + podaci[10] + "</div>" +

							"<a href='" + podaci[0] + ".html" + "' class='dis-block txt4 m-t-30'>" +
		...
		while (k < niz.length) {
			...
			if (podaci2[13] == podaci[13]) {
				...
				"<div class='col-md-8 col-lg-6 m-l-r-auto'>" +
					"<div class='blo3 flex-w flex-col-l-sm m-b-30'>" +
						"<div class='pic-blo3 size20 bo-rad-10 hov-img-zoom m-r-28'>" +
							"<a href='" + podaci2[0] + ".html" + "'><img src='" + podaci2[6] + "' alt='IMG-MENU' height='80%'></a>" +
						"</div>" +
						"<div class='text-blo3 size21 flex-col-l-m'>" +
							"<a href='" + podaci2[0] + ".html" + "' class='txt22 m-b-3'>" +
								podaci2[1] +
							"</a>" + 
							"<div><b>Трајање: </b>" + podaci2[7] + "</div>" +
							"<div><b>Режисер: </b>" + podaci2[8] + "</div>" +
							"<div><b>Улоге: </b>" + podaci2[10] + "</div>" +

							"<a href='" + podaci2[0] + ".html" + "' class='dis-block txt4 m-t-30'>" +
								"Прочитајте опширније" +
								"<i class='fa fa-long-arrow-right m-l-10' aria-hidden='true'></i>" +
							"</a>" +
							"<span class='txt22 m-t-20'>" + 
